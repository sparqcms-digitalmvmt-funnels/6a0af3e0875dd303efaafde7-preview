...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/6a0af3e0875dd303efaafde7-preview") ? value : (value.startsWith("/") ? "/6a0af3e0875dd303efaafde7-preview" + value : "/6a0af3e0875dd303efaafde7-preview/" + value);
  };
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "7-onWnrKDzPtrO2vEWrXVAutgHNGD2XM-eKp8rtYm_jAfMzvtuI0txw6c-5gN76k",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/6a0af3e0875dd303efaafde7-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
